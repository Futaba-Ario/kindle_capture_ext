...
5. この拡張機能のルートディレクトリ（例: `<project-root>`）を選択します。
...
## 公開向けドキュメント表記
- READMEやIssueにローカルの絶対パス（例: `C:\Users\...`）は書かず、`<project-root>` などのプレースホルダを使ってください。
- 端末コマンドを掲載する場合は、`cd <project-root>` のように環境依存しない表記を推奨します。
